# PoisonedOS Animation Assets

The `blocking`, `internal`, and `external` directories are the compiled PoisonedOS animation pack. The checked-in 1-bit PNG frames are generated deterministically from `docs/assets/poisoned_os.svg` and `docs/assets/poison.svg` by `tools/generate_poison_ui_assets.py`. `assets/SConscript` compiles internal and blocking animations into firmware and packages external animations for the SD card.

Regenerate the complete pack, PoisonedOS icons, level-up animations, and update slideshow from the repository root after entering the FBT environment:

```shell
source scripts/toolchain/fbtenv.sh
python3 tools/generate_poison_ui_assets.py
```

The generated pack follows the existing Flipper animation-file contract:

## Files

- `manifest.txt` contains the animation enumeration used for random selection.
- `meta.txt` describes how an animation is drawn.
- `frame_X.png` is an animation frame.
...
- `Min butthurt`, `Max butthurt` - range of the animation-state mood value.
- `Min level`, `Max level` - range of the animation-state level. If 0, this animation does not participate in random idle selection and can only be selected by exact name.
