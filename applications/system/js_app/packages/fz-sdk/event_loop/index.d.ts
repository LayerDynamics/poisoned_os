 ...
 * parameter after `subscribeStop` returns. `subscribe` also accepts optional
 * extra arguments when explicit per-subscription state is more convenient. The
 * callback can replace that state for its next invocation by returning an array
 * with one entry for each extra argument:
 * ```js
