...
static void esp_flasher_write_automation_status(EspFlasherApp* app, const char* status) {
    if(!app->automation_mode) return;

    File* file = storage_file_alloc(app->storage);
    const size_t length = strlen(status);
    bool written =
        storage_file_open(file, ESP_MARAUDER_STATUS_PATH, FSAM_WRITE, FSOM_CREATE_ALWAYS);
    if(written) {
        written = storage_file_write(file, status, length) == length && storage_file_sync(file);
    }
    storage_file_close(file);
    storage_file_free(file);
    if(!written) {
        loader_port_debug_print("Could not persist automation status\n");
    }
}
...
static esp_loader_error_t _flash_file(EspFlasherApp* app, char* filepath, uint32_t addr) {
    ...
    loader_port_debug_print("Verifying flash MD5\n");
    err = esp_loader_flash_verify();
    if(err != ESP_LOADER_SUCCESS) {
        snprintf(user_msg, sizeof(user_msg), "Flash verification failed with error %u\n", err);
        storage_file_close(bin_file);
        storage_file_free(bin_file);
        loader_port_debug_print(user_msg);
        return err;
    }
    loader_port_debug_print("Flash verified\n");
...
static esp_loader_error_t _switch_fw(EspFlasherApp* app, bool* switched) {
    *switched = app->switch_fw != SwitchNotSet;
...
static esp_loader_error_t _flash_all_files(EspFlasherApp* app) {
...
static int32_t esp_flasher_flash_bin(void* context) {
    ...
    esp_flasher_write_automation_status(app, "running\n");
    ...
    if(!err) {
        ...
        bool switched = false;
        err = _switch_fw(app, &switched);
        if(err == ESP_LOADER_SUCCESS && !switched) {
            err = _flash_all_files(app);
        }
        ...
        if(err == ESP_LOADER_SUCCESS) {
            ...
            char err_msg[96];
            snprintf(err_msg, sizeof(err_msg), "Flash failed with error: %u\n", err);
            loader_port_debug_print(err_msg);
        }
    }
    ...
    if(app->automation_mode) {
        char status[32];
        if(err == ESP_LOADER_SUCCESS) {
            strlcpy(status, "ok\n", sizeof(status));
        } else {
            snprintf(status, sizeof(status), "error:%u\n", err);
        }
        esp_flasher_write_automation_status(app, status);
        app->automation_complete = true;
    }
    return 0;
}
