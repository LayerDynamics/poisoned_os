...
static void animation_manager_switch_to_one_shot_view(AnimationManager* animation_manager) {
    ...
    if(stats.level == 1) {
        one_shot_view_start_animation(animation_manager->one_shot_view, &A_PoisonLevelup1_128x64);
    } else if(stats.level == 2) {
        one_shot_view_start_animation(animation_manager->one_shot_view, &A_PoisonLevelup2_128x64);
    } else {
        furi_crash();
    }
}
