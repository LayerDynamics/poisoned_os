...
static void poison_subghz_pair(void* context, bool level, uint32_t duration) {
    subghz_receiver_decode((SubGhzReceiver*)context, level, duration);
}
...
PoisonSubGhzHandle* poison_subghz_open(uint32_t frequency) {
    ...
    subghz_worker_set_context(handle->worker, handle->receiver);
...
bool poison_subghz_receive(
    ...
    result->rssi = subghz_devices_get_rssi(handle->device);
    result->lqi = subghz_devices_get_lqi(handle->device);
    const size_t size = furi_string_size(handle->decoded);
    if(size >= sizeof(result->decoded)) return false;
...
bool poison_subghz_transmit_raw(
    ...
    size_t count) {
    if(!handle || !handle->device || !timings || count == 0u || count > 1024u ||
    ...
    subghz_devices_set_frequency(handle->device, frequency);
    handle->frequency = frequency;
    if(!subghz_devices_set_tx(handle->device)) return false;
